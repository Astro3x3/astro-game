...
# WEAPON PICKUPS
# ==============================
WEAPON_AMMO = {"shotgun": 10, "rocket": 5, "laser": 10, "flamethrower": 10}
...
class WeaponPickup:
    ...
    def draw(self, surface, cam_x):
        ...
        icons = {"shotgun":"âŠ�?�","rocket":"âš¡","laser":"â�??¶","flamethrower":"â¬¡"}
...
class Rocket:
    ...
    def draw(self, surface, cam_x):
        ...
        if not self.activated:
            ...
            prompt = prompt_font.render("â�??¶ ENTER â�??�?�", True, NEON_YELLOW)
            shadow = prompt_font.render("â�??¶ ENTER â�??�?�", True, (80, 60, 0))
...
def draw_hud(hp, sc, kill_count, super_ready, super_stacks, mega_ready, level, super_active=0):
    ...
    for i in range(2):
        ...
        if banked:
            star = font_small.render("â�?�?�", True, WHITE)
    ...
    if super_active > 0:
        ...
        inv_lbl = font_small.render("â�?�?� INVINCIBLE â�?�?�", True, NEON_YELLOW)
    ...
    hint_text = "â�?� â�?��?? MOVE   â�?��?? JUMP   SPACE SHOOT   Q SUPER"
...
def draw_weapon_hud(surface, weapon, ammo):
    ...
    icons = {"blaster":"â�??�?�","shotgun":"âŠ�?�","rocket":"âš¡","laser":"â�??¶","flamethrower":"â¬¡"}
    ...
    if weapon != "blaster":
        ...
        inf = wf.render("â�?ž UNLIMITED", True, NEON_CYAN)
...
# Freed prisoners â�?��?? little pixel people with names
PRISONERS = [
...
CREDITS_LINES = [
    ("â�?�?�  ASTRO BOY: RND DEFENDER  â�?�?�",  (255,220,0),   36, True),
    ("",                                  WHITE,          18, False),
    ("",                                  WHITE,          18, False),
    ...
    ("",                                  WHITE,          18, False),
    ("",                                  WHITE,          18, False),
    ("â�?�?�  PRISONERS FREED: 8  â�?�?�",         (0,255,100),   26, True),
...
def draw_cutscene(surface, timer, final_score):
    """Full cutscene: boss explodes â�?��?? cage opens â�?��?? prisoners run free â�?��?? celebration."""
    tick = pygame.time.get_ticks()
...
while running:
    ...
    if game_state in ("over", "won") or game_over or game_won:
        ...
        if game_state == "won":
            t1 = font_big.render("â�?�?�  YOU WIN!  â�?�?�", True, NEON_YELLOW)
    ...
    if gs["level_complete"]:
        ...
        screen.blit(overlay, (0,0))
        msg = font_big.render("â�?�?�  LEVEL CLEAR!  â�?�?�", True, NEON_YELLOW)
    ...
    if gs["super_active"] > 0:
        ...
        lbl = font_big.render("â�?�?�  SUPER LASER  â�?�?�", True, NEON_YELLOW)
        sh  = font_big.render("â�?�?�  SUPER LASER  â�?�?�", True, (80,60,0))
    ...
    if gs["show_boss_warning"] and gs["boss_warning_timer"] > 0:
        if (gs["boss_warning_timer"]//8)%2==0:
            ...
            warn_txt = font_big.render("âš   BOSS INCOMING  âš ", True, NEON_RED)
    ...
    if gs["player_in_rocket"] and not rocket.launching:
        ...
        board_txt = font_big.render("ðŸš�?� LAUNCHING...", True, NEON_YELLOW)
